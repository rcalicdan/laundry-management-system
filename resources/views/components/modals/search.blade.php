...
<div x-show="isSearchModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div @click.away="isSearchModalOpen = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-auto"
        ...
        <div class="flex justify-between items-center pb-3 border-b">
            <p class="text-2xl font-bold">{{ $modalName }}</p>
        ...
        <div class="mt-6 flex justify-end space-x-2">
            <button @click="isSearchModalOpen = false; document.getElementById('search-term-input').value = ''"
                type="button" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition">
                Clear & Close
            </button>
            <button
                @click="isSearchModalOpen = false; alert('Performing search for: ' + document.getElementById('search-term-input').value + ' (Static Demo - would navigate or submit)')"
                type="button" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
                Search
            </button>
        </div>
    </div>
